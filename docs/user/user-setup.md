...
多账号 IM 配置需要同时闭合 config、route binding 和 Gateway runtime 三段链路；如果配置与 binding 已存在但对应 account runtime 未启动，消息不会进入该账号的会话。排查时优先检查 channel runtime、account enabled 状态、route binding 和 Gateway 日志。
