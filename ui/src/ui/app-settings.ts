import { roleScopesAllow } from "../metis-runtime/shared/operator-scope-compat.js";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as MetisApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as MetisApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as MetisApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  ...
  if (host.tab === "agents") {
    ...
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as MetisApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as MetisApp, agentId);
      if (host.agentsPanel === "files") {
        void loadAgentFiles(host as unknown as MetisApp, agentId);
      }
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as MetisApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as MetisApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as MetisApp);
  ...
  if (host.tab === "dreams") {
    await loadConfig(host as unknown as MetisApp);
    await Promise.all([
      loadDreamingStatus(host as unknown as MetisApp),
      loadDreamDiary(host as unknown as MetisApp),
    ]);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as MetisApp);
    await loadConfig(host as unknown as MetisApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as MetisApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as MetisApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__METIS_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as MetisApp;
...
async function loadOverviewLogs(host: MetisApp) {
...
function buildAttentionItems(host: MetisApp) {
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as MetisApp, true),
    loadConfigSchema(host as unknown as MetisApp),
    loadConfig(host as unknown as MetisApp),
  ]);
}
