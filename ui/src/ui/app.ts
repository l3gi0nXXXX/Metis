...
declare global {
  interface Window {
    __METIS_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
@customElement("metis-app")
export class MetisApp extends LitElement {
  ...
  protected shouldUpdate(changed: Map<PropertyKey, unknown>) {
    return shouldMetisAppRenderForChangedKeys(
      this.tab,
      [...changed.keys()].map((key) => String(key)),
    );
  }
